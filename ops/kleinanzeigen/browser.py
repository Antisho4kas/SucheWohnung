...
def _proxy_from_env():
    """Build a Playwright proxy config from the PROXY env var, if set.

    Supports http(s):// and socks5:// URLs with optional user:pass, e.g.
    http://user:pass@host:port  or  socks5://host:port. Returns None when
    PROXY is unset so the browser launches directly (unchanged behaviour).
    """
